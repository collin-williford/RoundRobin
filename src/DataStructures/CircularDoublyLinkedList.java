...
public class CircularDoublyLinkedList<T extends Comparable<T>> implements ListADT<T> {
    ...
    */
    ...
    public void addLast(T element) {
        ...
        DoublyLinkedNode<T> newLast = new DoublyLinkedNode(element);
        ...
        if (first == null) { // case 1
            ...
            newLast.setPrev(first.getPrev().getPrev());
...
//        first.setNext(first.getPrev().getNext());
          first = first.getPrev();
    ...
    public void addAfter(T existing, T element) throws ElementNotFoundException, EmptyCollectionException {
        DoublyLinkedNode<T> foundNode = find(existing); //  new node which holds the reference to the given element 
        
        //TODO create a node and maintain all references to 
        //insert the new node after the found node.
        if (foundNode == null) 
            throw new ElementNotFoundException("CircularDoublyLinkedList - Existing element not found for addAfter");
        if (isEmpty()) {
    ...
    public T remove(T element) throws EmptyCollectionException, ElementNotFoundException {
        ...
        if(foundNode == first) {
            first.setNext(null);
            first.setPrev(null);
            first =null;   // this might need to 
            numNodes--;
            
        }
        if (foundNode.getNext() == first) {
            first.setNext(first);
            first.setPrev(first);
            ...
            foundNode.setNext(null);
            foundNode.setPrev(null);
            //foundNode = null;    // unsure if this is needed since you still need to return its element 
            numNodes--;
    ...
    public T removeLast() throws EmptyCollectionException {
        ...
        DoublyLinkedNode<T> temp = new DoublyLinkedNode(first.getPrev());
        if (this.numNodes == 1) {
            first.setNext(null);
            first.setPrev(null);
            first = null;
    ...
    public boolean contains(T element) throws EmptyCollectionException {
        ...
        DoublyLinkedNode<T> curr = new DoublyLinkedNode(first.getNext().getNext());
        
        if ((first.getElement() == element)) {
            return true;
        }
        //T temp = first.getElement();
        if (numNodes ==  2) {    // first.getPrev() won't work for some reason when referencing the 2nd node so I have to do this 
            if ((first.getNext().getElement() == element)) {
                return true;
            }
        }
        if (numNodes >=3) {
